...
module.exports = async (req, res, next) => {
  ...
  try {
    const results = await Promise.all([
      ...
      Account.findOne({ where: { accountNumber: creditAccountNumber }, transaction: t })
    ]);
    if (results[0] === null) {
      return res.status(400).json({ message: 'User has no account with Nairalink' });
    };
    ...
    if (debitAccount.balance < amount) {
      return res.status(400).json({ message: 'Insufficient funds to make this transaction. Top up!' });
    };
    ...
    const debit = await Account.increment({ balance: -amount }, { where: { accountNumber: debitAccount.accountNumber }, transaction: t });
    console.log(debit);
    if (debit[0][-1] === 1) {
      const credit = await Account.increment({ balance: amount}, { where: { accountNumber: creditAccountNumber }, lock: true, transaction: t});
      console.log(credit);
      if (credit[0][-1] === 1) {
        await t.commit();
        ...
        throw new Error('Error encountered while crediting receiver account:', credit[0][0]);
      }
    }
    throw new Error('Error encountered while debiting sender account:', debit[0][0]);
  } catch (error) {
    console.log(error);
    await t.rollback();
    if (transact) {
